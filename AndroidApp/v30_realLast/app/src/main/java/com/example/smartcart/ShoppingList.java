...
//                }
                ...
                builder.setTitle("구매창").setMessage("구매를 하시겠습니까?");
                ...
                builder.setPositiveButton("예", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        for(int i=0; i<orderList.size();i++){
                            ShoppingListRequest shoppingListAdapter = new ShoppingListRequest(orderList.get(i), responseListener);
                            RequestQueue queue = Volley.newRequestQueue(ShoppingList.this);
                            queue.add(shoppingListAdapter);
                            Log.d("Response","orderList.get(0)"+orderList.get(i));
                        }
...
//                shoppingListAdapter shoppingListAdapter = new shoppingListAdapter(orderList.get(0), responseListener);
//                RequestQueue queue = Volley.newRequestQueue(ShoppingList.this);
//                queue.add(shoppingListAdapter);
                Log.d("Response","orderList.get(0)"+orderList.get(0));
    ...
    void readBarcode() {
        while (true) {
            try {
                ...
                //바코드 값을 상품명과 상품가격으로 변환하기 위한 메소드 실행
                new Thread() {
                    public void run() {
                        getInfo();
                    }
                ...
                try {
                    ...
                    if (success.length() > 0) {
                        ...
                        for (int i = 0; i < obj.length(); i++) {
                            ...
                            result1 += user.getString("price")+"원\n";
                            ...
                            orderTemp = UserInfo.id + " " +db_value_pdInfo+" "+db_value_pdInfo1+" "+currentTime;
