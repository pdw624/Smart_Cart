...
*/
    public void onDestroy() { // 앱이 소멸되면 pc에 신호 보냄
        super.onDestroy();
        if (cSocket != null) {
            sendMessage("# [" + usingName + "]님이 나가셨습니다.");
        }
    }
        ...
        public void run(){
            ...
            try {
                while (!flag) { // 플래그가 false일경우에 루프
                    ...
                    if (msgs.equals("# [" + usingName + "]님이 나가셨습니다.")) { // 서버에서 온 메세지가 종료 메세지라면
